...
public class NeighboringSkillMutator3 extends Mutator<SkillGene, Float>
{
	private final PassiveSkillTree pst;

	public NeighboringSkillMutator3(final float mutationRate, final PassiveSkillTree pst)
	{
		super(mutationRate);
		this.pst = pst;
	}
	...
	protected int mutate(final MSeq<SkillGene> genes, final double p)
	{
		...
		// final Integer startPoint = original.get(random.nextInt(original.size()));
		// final Set<Integer> walk = pst.randomWalk(startPoint, 10, random);

		final AtomicInteger atom = new AtomicInteger(-1);
		return (int)indexes(RandomRegistry.getRandom(), genes.length(), RandomRegistry.getRandom().nextFloat())
				.peek(i -> {
					if (atom.get() == -1)
					{
						...
						// atom.set(genes.get(0).getAllowedSkills().get(random2.nextInt(genes.get(0).getAllowedSkills().size())));
						// }
						// else
						// {
