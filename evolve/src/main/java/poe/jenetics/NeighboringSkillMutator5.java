...
public class NeighboringSkillMutator5 extends Mutator<SkillGene, Float>
{
	private final PassiveSkillTree pst;

	public NeighboringSkillMutator5(final float probability, final PassiveSkillTree pst)
	{
		super(probability);
		this.pst = pst;
	}
			...
			final Random random)
	{
		...
		final ISeq<MutatorResult<SkillGene>> result = chromosome.toSeq()
				.map(gene -> random.nextInt() < P
						? MutatorResult.of(mutate2(gene, random, chromosome.getGene(random.nextInt(chromosome.length()))), 1)
	...
	private SkillGene mutate2(final SkillGene gene, final Random random, final SkillGene neighbor)
	{
		return gene.newInstance(pst.randomNeighbor(neighbor.getAllele(), random));
	}
